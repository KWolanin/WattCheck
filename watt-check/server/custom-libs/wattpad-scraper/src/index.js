...
class WattpadScraper {
    ...
    async getParts(url) {
        try {
            ...
            const tableOfContents = $('[data-testid="toc"] ul[aria-label="story-parts"]');

            const storyParts = [];

            tableOfContents.find('li').each((_, element) => {
            const $element = $(element);
            const title = $element.find('a > div').first() 
                .children('div').eq(0)                      
                .children('div').eq(1)                       
                .text()
                .trim()
            const date = $element.find('a').children('div').eq(1).text().trim()
            const link = $element.find('a').attr('href');
            storyParts.push({ title, link, date });
            });
            return storyParts;
        } catch (error) {
